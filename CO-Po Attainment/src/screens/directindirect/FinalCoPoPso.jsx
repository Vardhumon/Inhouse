          ...
          <div className="vw-75  overflow-auto">
            <div className=" bg-light text-dark px-4 py-3 border border-3">
              <table className="table table-bordered h-75" style={{ minHeight: "200px" }}>
                ...
                <tbody>
                  {
                    data.DirectPo80perc.map((item, index) => (
                      <tr key={index}>
                        <td className="text-center" colSpan={4}>80% of Direct Attainment</td>
                    ...
                    {
                        data.IndirectPo20perc.map((item,index) => (
                            <tr key={index}>
                        <td className="text-center" colSpan={4}>20% of In-Direct Attainment</td>
                        {item.po.map((value, idx) => (
                          <td className="text-center" style={{ minWidth: "60px" }} key={idx}>{value}</td>
                        ))}
                        {item.pso.map((value, idx) => (
                    ...
                    {
                        data.COPOPSOAttainment.map((item,index) => (
                            <tr key={index}>
                        <td className="text-center" colSpan={4}>CO PO-PSO Attainment</td>
                        {item.po.map((value, idx) => (
                          <td className="text-center" style={{ minWidth: "60px" }} key={idx}>{value}</td>
                        ))}
                        {item.pso.map((value, idx) => (
